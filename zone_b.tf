...
// Control node
resource "aws_instance" "control_node_b" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo kubeadm join --token ${var.cluster_control_join_token} ${aws_instance.control_node_a[0].private_ip}:6443"
    ]
  }
...
resource "aws_instance" "worker_node_b" {
  ...
  tags = {
    Name = "Worker node b instance ${count.index}"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo kubeadm join --token ${var.cluster_worker_join_token} ${aws_instance.control_node_b[0].private_ip}:6443"
    ]
  }
